...
func View_user_setting(config tool.Config, values url.Values) string {
	...
	body += `<div><a href="/change/head">(` + tool.Get_language(db, "user_head", false) + `)</a></div><hr class="main_hr"><div><a href="/change/top_menu">(` + tool.Get_language(db, "user_added_menu", true) + `)</a></div><hr class="main_hr"><span>` + tool.Get_language(db, "skin", true) + `</span><hr class="main_hr"><div><select name="skin">` + skin_options(current_skin) + `</select></div><hr class="main_hr">`
	body += `<div><a href="/change/skin_set">(` + tool.Get_language(db, "skin_set", true) + `)</a></div><hr class="main_hr"><div><a href="/change/skin_set/main">(` + tool.Get_language(db, "main_skin_set", true) + `)</a></div><hr class="main_hr"><span>` + tool.Get_language(db, "language", true) + `</span><hr class="main_hr"><div><select name="lang">` + language_options(current_language) + `</select></div><hr class="main_hr"><span>` + tool.Get_language(db, "user_title", true) + `</span><hr class="main_hr"><div><select name="user_title">` + title_options + `</select></div><hr class="main_hr"><h2>` + tool.Get_language(db, "2fa", true) + `</h2><div><select name="2fa">` + twofa_options + `</select></div><hr class="main_hr"><div><input type="password" name="2fa_pw" placeholder="` + tool.Get_language(db, twofa_password, true) + `"></div><hr class="main_hr"><h2>` + tool.Get_language(db, "main_user_name", true) + `</h2><div><a href="/change/user_name">(` + tool.Get_language(db, "change_user_name", true) + `)</a></div><hr class="main_hr">`
	body += `<h2>` + tool.Get_language(db, "profile_image", true) + `</h2><div><input name="profile_image" value="` + tool.HTML_escape(profile_image) + `" placeholder="file_name.png"></div><div>` + tool.Get_language(db, "profile_image_help", true) + `</div><hr class="main_hr">`
	body += `<span>` + tool.Get_language(db, "online_status", true) + `</span><hr class="main_hr"><div><select name="online_status">` + online_status_options + `</select></div><hr class="main_hr">`
	body += `<div>` + tool.Get_language(db, "user_name", true) + ` : ` + tool.HTML_escape(user_name) + `</div><hr class="main_hr"><h2>` + tool.Get_language(db, "sub_user_name", true) + `</h2><div><input name="sub_user_name" value="` + tool.HTML_escape(User_value(db, config.IP, "sub_user_name")) + `" placeholder="` + tool.Get_language(db, "sub_user_name", true) + `"></div><hr class="main_hr"><div><button type="submit">` + tool.Get_language(db, "save", true) + `</button></div>` + tool.Get_http_warning(db) + `</form>`
	return User_form_page(db, config, tool.Get_language(db, "user_setting", true), body)
}
