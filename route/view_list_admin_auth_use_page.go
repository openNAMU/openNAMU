...
func View_list_admin_auth_use_page(config tool.Config, page string, search string) string {
	...
	return List_extra_page(db, config, tool.Get_language(db, "auth_use", true), `<form method="post"><div><label for="auth_use_search">`+tool.Get_language(db, "search", true)+`</label><input id="auth_use_search" name="search" value="`+tool.HTML_escape(search)+`"></div><hr class="main_hr"><div><button type="submit">`+tool.Get_language(db, "search", true)+`</button></div></form><hr class="main_hr">`+body.String())
}
