...
func Get_template_set(skin_name string) map[string]string {
	...
	if err == nil {
		...
		if err := json.Unmarshal(data, &set_json); err != nil {
...
func Get_wiki_css(data []any, cookies string) []any {
	...
	data_css += `<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/monaco-editor/0.56.0/min/vs/editor/editor.main.min.css" integrity="sha512-cVxx6lu+ezXIqsdxOJI9fgTfJ6BXNBrWb1nVGrVCEnq3TT8ldhLWOvfglyqDxAuB4bGnH/zeCsKBT7c9U1OrjQ==" crossorigin="anonymous" referrerpolicy="no-referrer" />`
...
func Get_error_page(db *sql.DB, config Config, error_name string) string {
	...
	case "edit time":
		data = Get_language(db, "edit_time_error", true)
	...
	case "bbs title length":
		data = Get_language(db, "bbs_title_length_error", true)
	case "bbs tag length":
		data = Get_language(db, "bbs_tag_length_error", true)
	case "comment_closed":
		data = Get_language(db, "comment_closed", true)
