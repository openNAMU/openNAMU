...
func View_user_password(config tool.Config, values url.Values) string {
	...
	body := `<form method="post"><label for="password_now">` + tool.Get_language(db, "now_password", true) + `</label> <input id="password_now" placeholder="` + tool.Get_language(db, "now_password", true) + `" name="password_now" type="password"><hr class="main_hr"><label for="password_new">` + tool.Get_language(db, "new_password", true) + `</label> <input id="password_new" placeholder="` + tool.Get_language(db, "new_password", true) + minimum_text + `" name="password_new" type="password"><hr class="main_hr"><label for="password_new_repeat">` + tool.Get_language(db, "password_confirm", true) + `</label> <input id="password_new_repeat" placeholder="` + tool.Get_language(db, "password_confirm", true) + `" name="password_new_repeat" type="password"><hr class="main_hr"><button type="submit">` + tool.Get_language(db, "save", true) + `</button>` + tool.Get_http_warning(db) + `</form>`
