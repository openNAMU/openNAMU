...
func View_bbs_edit_post(config tool.Config, set_id string, set_code string, comment_code string, title string, data string, prefix string, captcha string) string {
	db := tool.DB_connect()
	...
	api_data := Api_bbs_w_edit_post(config, set_id, set_code, comment_code, title, data, prefix)
