...
def list_block_2(name, tool):
    ...
    if not name:        
        if flask.request.args.get('type', '') == 'ongoing':
            ...
            sqlQuery('select why, block, '', end, '', band from ban where ((end > ? and end like "2%") or end = "") order by end desc limit ?, 50', [get_time(), sql_num])
        else:
            sub = 0
            menu = 0
            ...
            sqlQuery('select why, block, blocker, end, today, band from rb order by today desc limit ?, 50', [sql_num])
    else:
        menu = [['block_log', load_lang('normal')]]
        ...
        if tool == 'block_user':
            ...
            sqlQuery('select why, block, blocker, end, today, band from rb where block = ? order by today desc limit ?, 50', [name, sql_num])
        else:
            sub = ' (' + load_lang('admin') + ')'
            
            sqlQuery('select why, block, blocker, end, today, band from rb where blocker = ? order by today desc limit ?, 50', [name, sql_num])
