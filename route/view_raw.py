...
def view_raw_2(name, sub_title, num):
    ...
    if not sub_title and num:
        ...
        if admin_check(6) != 1:
            sqlQuery("select data from topic where id = ? and title = ? and sub = ? and block = ''", [str(num), name, sub_title])
        else:
            sqlQuery("select data from topic where id = ? and title = ? and sub = ?", [str(num), name, sub_title])
