...
def topic_close_list_2(name, tool):
    ...
    if flask.request.method == 'POST':
        ...
        while 1:
            sqlQuery("select title from topic where title = ? and sub = ? limit 1", [name, flask.request.form.get('topic', None) + t_num])
        ...
        if tool == 'close':
            sqlQuery("select sub from rd where title = ? and stop = 'O' order by sub asc", [name])
            
            sub = load_lang('closed_discussion')
        elif tool == 'agree':
            sqlQuery("select sub from rd where title = ? and agree = 'O' order by sub asc", [name])
            
            sub = load_lang('agreed_discussion')
        else:
            sqlQuery("select sub from rd where title = ? order by date desc", [name])
        ...
        for data in sqlQuery("fetchall"):
            ...
            sqlQuery("select data, date, ip, block from topic where title = ? and sub = ? and id = '1'", [name, data[0]])
            if sqlQuery("fetchall"):                
                it_p = 0
                
                if sub == load_lang('discussion_list'):
                    sqlQuery("select title from rd where title = ? and sub = ? and stop = 'O' order by sub asc", [name, data[0]])
                    if sqlQuery("fetchall"):
                        it_p = 1
                
                if it_p != 1:
                    sqlQuery("select id from topic where title = ? and sub = ? order by date desc limit 1", [name, data[0]])
