...
func View_login_2fa(config tool.Config, values url.Values) string {
	...
	body := `<form method="post"><div><label for="two_factor_password">` + tool.Get_language(db, "2fa_password", true) + `</label> <input id="two_factor_password" type="password" name="pw"></div><hr class="main_hr">` + tool.Get_captcha_ui(db, config) + `<div><button type="submit">` + tool.Get_language(db, "login", true) + `</button></div>` + tool.Get_http_warning(db) + `</form>`
