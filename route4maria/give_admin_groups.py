...
def give_admin_groups_2(conn, name):
    ...
    if flask.request.method == 'POST':
        ...
        curs.execute("delete from alist where name = %s", [name])
        
        if flask.request.form.get('ban', 0) != 0:
            curs.execute("insert into alist (name, acl) values (%s, 'ban')", [name])

        if flask.request.form.get('toron', 0) != 0:
            curs.execute("insert into alist (name, acl) values (%s, 'toron')", [name])
            
        if flask.request.form.get('check', 0) != 0:
            curs.execute("insert into alist (name, acl) values (%s, 'check')", [name])

        if flask.request.form.get('acl', 0) != 0:
            curs.execute("insert into alist (name, acl) values (%s, 'acl')", [name])

        if flask.request.form.get('hidel', 0) != 0:
            curs.execute("insert into alist (name, acl) values (%s, 'hidel')", [name])

        if flask.request.form.get('give', 0) != 0:
            curs.execute("insert into alist (name, acl) values (%s, 'give')", [name])
        ...
        curs.execute('select acl from alist where name = %s', [name])
