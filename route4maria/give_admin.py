...
def give_admin_2(conn, name):
    ...
    if not user:
        ...
        if owner != 1:
            curs.execute('select name from alist where name = %s and acl = "owner"', [user[0][0]])
            if curs.fetchall():
    ...
    if flask.request.method == 'POST':
        ...
        if owner != 1:
            curs.execute('select name from alist where name = %s and acl = "owner"', [flask.request.form.get('select', None)])
            if curs.fetchall():
                return re_error('/error/3')

        if flask.request.form.get('select', None) == 'X':
            curs.execute("update user set acl = 'user' where id = %s", [name])
        else:
        ...
        for data in curs.fetchall():
            if user[0][0] == data[0]:
                ...
                if owner != 1:
                    curs.execute('select name from alist where name = %s and acl = "owner"', [data[0]])
