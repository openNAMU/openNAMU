...
func View_thread_comment_tool(config tool.Config, topic_num string, comment_num string) string {
	...
	if tool.Check_permission(db, "give", config.IP) {
		data += `<h2>` + tool.Get_language(db, "admin_tool", true) + `</h2><ul><li><a href="/auth/give/` + tool.Url_parser(ip) + `">` + tool.Get_language(db, "authorize", true) + ` | ` + tool.Get_language(db, "auth_release", true) + `</a></li></ul>`
	}
