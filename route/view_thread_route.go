...
func View_thread_route(config tool.Config, topic_num string, doc_name string, page string, values url.Values) string {
	db := tool.DB_connect()
	if values != nil {
	...
	return View_thread(config, topic_num, doc_name, page, values)
}
